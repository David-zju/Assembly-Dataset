...
- 直接复用已经由 `refine-cylindrical-contact-overlap` 实现的 `src/l1_contact_detection/cylindrical_overlap.py`，避免重复实现圆柱 UV/domain 逻辑。
...
2. 对每个 WIRE 遍历 EDGE。
3. 将 EDGE 的 3D 边界点或采样点投影到 `PlaneFrame` 的 2D 坐标。
4. 初始 Level 2 实现面向凸边界：对每个 wire 收集边界点后按 centroid 极角排序，并检查凸性；以投影后绝对面积最大的闭合 ring 作为 outer boundary，其余 ring 作为 holes。
...
- Fallback：有孔、多 outer、非闭合 wire、非凸边界、BSPLINE/ELLIPSE 等暂未验证边界，降级为 bbox/诊断，不作为高置信度有限面 overlap。
...
### 决策 6：Tangency 直接复用已实现的圆柱 trimmed domain 工具

**选择**：Tangency 的圆柱有限域检查直接复用已实现的 `src/l1_contact_detection/cylindrical_overlap.py`。可用接口包括：
- `CylinderFrame`
- `CylinderTrimDomain`
...
`cylindrical_overlap.py` 当前已经提供“两段圆柱 domain 之间的周向 overlap”能力；Tangency 还需要补充一个轻量 helper：`cylinder_domain_contains_angle(domain, angle, tolerances)`，用于判断理论切线对应的 fixed tangent angle 是否落入 `CylinderTrimDomain.angular_intervals`。该 helper 应放在同一模块中，供 Tangency 调用，也便于后续其他圆柱相关接触复用。

**原因**：Tangency 与 Cylindrical 都需要从 CYLINDER face 提取有限 UV/domain。复用已实现的同一工具可减少两个判定器在周期角、跨零区间、非全局轴线方面产生不一致。新增 fixed-angle helper 是 Tangency 的特殊需求，不应在 `tangency_contact.py` 内部临时重复实现。
...
这些配置进入 `Tolerances`。圆柱侧采样与周向判断复用已经存在的 `cylinder_domain_sample_count`、`full_circle_angle_tol_deg`、`min_circumferential_overlap_ratio`、`min_circumferential_overlap_deg`，不得在本变更中重新定义第二套圆柱侧配置。`bbox_overlap_min_ratio` 可保留给 fallback 或历史诊断字段，但不再作为 Planar 主路径的主要阈值。
...
- **[R4] 凸多边形算法无法覆盖所有平面 trimmed face** → 当前实现对边界采样点做极角排序并检查凸性；非凸、含孔或复杂曲线 domain 走 fallback，不把复杂结果伪装成精确 overlap。
- **[R5] Tangency 依赖已实现圆柱 domain 工具的 fixed-angle 能力** → 复用 `cylindrical_overlap.py` 已有 frame/domain 提取函数，并在该模块补充 `cylinder_domain_contains_angle()`，避免在 Tangency 中重复实现角度区间逻辑。
