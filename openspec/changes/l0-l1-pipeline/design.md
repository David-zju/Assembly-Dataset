...
**Goals:**
- 实现 STEP 装配体文件的导入与扁平化，正式装配体导入以 `cq.Assembly.load()` 保留 Part 边界，`importStep + TopExp_Explorer` 仅作为几何诊断兜底
...
### 决策 1.1：装配体 Part 边界以 Assembly.load 为唯一可靠来源

**选择**：L0 正式装配体导入 SHALL 以 `cq.Assembly.load()` 解析出的装配树作为 Part 边界来源。`cq.importers.importStep()` MAY 作为几何诊断兜底，但其返回的 `Compound` / `solids()` 不得被解释为可靠 Part。

**原因**：已验证 `importStep()` 对 `test_case/装配体3.STEP` 和 `test_case/机器狗.STEP` 返回单个 `Compound`，虽然可提取 298 个 solids 和 19759 个 faces，但丢失装配树、组件名称、实例路径和位姿语义。solid 不等价于 Part：一个 Part 可能包含多个 solid，同一个零件定义也可能有多个装配实例。L1 依赖跨 Part 判定，错误的 Part 边界会直接污染 FaceContact。

**降级规则**：
- `Assembly.load()` 成功并可从 leaf Part 提取 face → `import_strategy = "assembly_load"`，`part_boundary_reliable = true`，允许继续 L1
- `Assembly.load()` 失败或无法提取有效 Part face，但 `importStep()` 可提取几何 → 允许生成单个 synthetic Part 用于 L0 几何诊断，`import_strategy = "import_step_fallback"`，`part_boundary_reliable = false`，不得继续执行 L1 跨 Part 接触检测
- 两种方式均无法提取 face → 抛出 `StepImportError`
...
### 决策 3.1：身份映射保留所有拓扑 face

**选择**：L0 对 TopExp_Explorer 遍历到的每一个拓扑 face 都分配 face_uid、记录 `global_face_index` / `part_face_index`、计算几何指纹并写入 L0 输出。不支持 L1 接触检测的几何类型（如 BEZIER / BSPLINE / OTHER）不得从 L0 身份序列中删除，只能标记 `supported = false` 和 `skip_reason`。

**原因**：L1 跨进程恢复依赖完整遍历顺序。如果 L0 在中间跳过 unsupported face，而 L1 重新导入后仍遍历完整拓扑 face，后续 face_uid 会整体错位。身份层必须完整保留，检测层再过滤。

**使用规则**：
- `face_uid → cq.Face` 内存映射覆盖所有拓扑 face
- L1 面分类、空间索引和接触判定只使用 `supported = true` 的 face
- unsupported face 仍参与指纹校验和 UID 引用完整性校验，但不参与 FaceContact 生成
- skipped_face_count 表示 `supported = false` 的数量，不表示这些 face 从 L0 输出中被删除
...
- **[R1] Assembly.load 性能与兼容性**：`simple_l0_l1_assembly.step` 可作为小型有效装配验证件；大型 STEP 上 `Assembly.load()` 可能较慢，需记录耗时并提供诊断输出；`importStep()` 只能作为几何兜底，不能提供可靠 Part 边界
