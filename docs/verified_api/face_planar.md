...
- 真实 STEP 中 `surf` 可能是 `Geom_RectangularTrimmedSurface`，需要先用
  `surf.BasisSurface()` 解包到底层 `Geom_Plane`，再调用 `Pln()`。
...
部分 STEP 文件会把平面底层曲面包在 `Geom_RectangularTrimmedSurface` 中。
这种 wrapper 本身没有 `Pln()`，需要先递归解包：
...
### 验证场景 3：Geom_RectangularTrimmedSurface 包裹平面

**日期**：2026-05-29
**模型/数据**：使用 `Geom_RectangularTrimmedSurface(Geom_Plane(...), u1, u2, v1, v2)` 构造被 wrapper 包裹的平面。
**代码**：
```python
import cadquery as cq
from OCP.BRep import BRep_Tool
...
base = Geom_Plane(gp_Ax3(gp_Pnt(0, 0, 0), gp_Dir(0, 0, 1)))
trimmed = Geom_RectangularTrimmedSurface(base, -5, 5, -4, 4)
face = cq.Face(BRepBuilderAPI_MakeFace(trimmed, 1e-7).Face())
surf = BRep_Tool.Surface_s(face.wrapped)

print(type(surf).__name__)          # Geom_RectangularTrimmedSurface
print(hasattr(surf, "Pln"))         # False
basis = surf.BasisSurface()
print(type(basis).__name__)         # Geom_Plane
print(basis.Pln().Location().Z())   # 0.0
```
**预期行为**：wrapper 可通过 `BasisSurface()` 解包，底层平面保留 `gp_Pln` 参数。
**实际结果**：符合预期；未解包时 `Geom_RectangularTrimmedSurface` 没有 `Pln()`。
**结论**：L1 平面几何提取必须统一解包 `BasisSurface()`，不能假设 `Surface_s()` 直接返回 `Geom_Plane`。
